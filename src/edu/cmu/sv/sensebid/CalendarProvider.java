...
public class CalendarProvider {
	...
	public ArrayList<Reservation> readCalendarEvents(Context context) {
		String uri = "content://com.android.calendar/events"; // Check for
																// updated APIs
		...
		for (int i = 0; i < cursor.getCount(); i++) {
			...
			if (isTodayEventAndHalfAnHourLaterThanNow(eventCalendar) == true
					|| isTomorrowEvent(eventCalendar) == true) {
	...
	private static boolean isTodayEventAndHalfAnHourLaterThanNow(
		...
		if (today.getTime() > event.getTime()) {
			...
			long minutesBetweenEvents = (int) (((event.getTime() - today
					.getTime()) / 1000) / 60);

			// Adding 30 minutes to event so user is able to bid for temperature
			eventCalendar.add(Calendar.MINUTE, 30);

			if (eventCalendar.get(Calendar.YEAR) == todayCalendar
					.get(Calendar.YEAR)
					&& eventCalendar.get(Calendar.MONTH) == todayCalendar
							.get(Calendar.MONTH)
					&& eventCalendar.get(Calendar.DAY_OF_MONTH) == todayCalendar
	...
	private static boolean isTomorrowEvent(Calendar eventCalendar) {
		...
		if (eventCalendar.get(Calendar.YEAR) == tomorrowCalendar
				.get(Calendar.YEAR)
				&& eventCalendar.get(Calendar.MONTH) == tomorrowCalendar
						.get(Calendar.MONTH)
				&& eventCalendar.get(Calendar.DAY_OF_MONTH) == tomorrowCalendar
						.get(Calendar.DAY_OF_MONTH)) {
			return true;
		} else {
			return false;
